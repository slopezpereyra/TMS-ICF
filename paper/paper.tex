...
types, as shown in Figures \ref{fig:figure1} and \ref{fig:figure2}.
...
Each of the means in the above table corresponds to the estimated $\beta$
...
observation with the appropriate subject label. In other words, its goal is to
infer, based on the properties of each TMS response, the group of the subject
upon which the stimulation was elicited, as well as the type of sleep session
after which it occurred. Our hypothesis is that pulse-specific measures of relative
...
When trained on the raw data ---that is, without the inclusion of pulse-specific
MRA--- the model's accuracy was of $\approx34.2 \%$. However, the confusion
matrix of the model, \textbf{Figure 3}, shows the errors are concentrated on the healthy control
...
We then trained the model first with the aggregation of the $\rho$ feature, then
with the aggregation of the $\delta$ feature, and then with the aggregation of
both.

With the inclusion of the $\rho$ feature increased accuracy by a factor of
$\approx 2.1$ to $72.6\%$. The inclusion of the $\delta$ feature had a similar
impact, increasing accuracy to $73.4\%$.
...
The inclusion of both the $\delta$ and $\rho$ features computed over the
logarithmically transformed EMG peak-to-peak improved the model's accuracy.
Concretely, accuracy was increased to $86\%$, with the following confusion
matrix.

\subsection{Random forest after $\ln$ transforms and weighted averages}
...
performance of a random forest model (see \textbf{Empirical results}).
...
As stated in section \textbf{3}, we experimented with using weighted
versions of our pulse-specific MRA. They consisted in weighting test pulses
with some appropriate weight vector. For example, including $\rho$ and
$\delta$ using inverse-variance weights, we obtained an accuracy of
$90.2\%$, with the following confusion matrix.

\begin{figure*}
    \centering
    \includegraphics[scale=0.4]{final-cm}
    \caption{Confusion matrix: $\rho(x)$, $\delta(x), \rho(\ln x), \delta(\ln
    x)$, $\rho_w(x)$ and $\delta_w$(x).}
    \label{fig:figure5}
\end{figure*}

\section{Discussion}

The results in \textbf{Section 6} demonstrate that pulse-specific MRA greatly
improve the accuracy of a random forest model trained over TMS data for subject
...
use of making machine learning models over TMS experimental data. Such models
can play an important part in future research by introducing new ways of
both analyzing and extracting meaningful insights from TMS results.
